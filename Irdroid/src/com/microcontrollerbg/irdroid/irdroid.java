...
public class irdroid extends Activity {
	...
	public int bufSize = AudioTrack.getMinBufferSize(48000,AudioFormat.CHANNEL_CONFIGURATION_STEREO,AudioFormat.ENCODING_PCM_8BIT);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		apple_next.setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View view, MotionEvent motionevent) {
				...
				if (action == MotionEvent.ACTION_DOWN) {
					...
					mHandler.postAtTime(next, SystemClock.uptimeMillis() + 250);
		...
		apple_prev.setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View view, MotionEvent motionevent) {
				...
				if (action == MotionEvent.ACTION_DOWN) {
					...
					mHandler.postAtTime(prev, SystemClock.uptimeMillis() + 250);
		...
		apple_volup.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View view, MotionEvent motionevent) {
				...
				if (action == MotionEvent.ACTION_DOWN) {
					...
					mHandler.postAtTime(volup, SystemClock.uptimeMillis() + 250);
		...
		// spinCommand.getSelectedItem() == null) {
		// Toast.makeText(getApplicationContext(),
		// "Please select a device and a command", Toast.LENGTH_SHORT).show();
		// return;
		// }
	...
	public void updateDeviceList() {
		...
		if (str == null) {
			Toast.makeText(getApplicationContext(),
					"Invalid, empty or missing config file", Toast.LENGTH_SHORT)
					.show();
			// selectFile();
			return;
		}
		...
		// Log.e("ANDRPOLIRC",
		// "Device list successfuly updated. Number of devices: "+String.valueOf(str.length)+"---"+deviceList.getItem(1));
	}

	void sendSignal(String device, String cmd) {

		// buffer=null;
		buffer = new byte[8];//lirc.getIrBuffer(device, cmd);
		...
		// try {
		// FileOutputStream myOutput = new
		// FileOutputStream("/data/data/com.zokama.androlirc/power_toggle.raw");
	...
	void deleteConfigFile() {
		...
		if (!file.exists())
			...
			Toast.makeText(getApplicationContext(), "Couldn't delete the file",
					Toast.LENGTH_SHORT).show();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK && requestCode == 1) {
			...
			// updateDeviceList();
			// Toast.makeText(getApplicationContext(), "Doobre",
			// Toast.LENGTH_SHORT).show();
		}
	}
